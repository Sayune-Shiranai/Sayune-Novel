...
import { getBookBySlug, updateBook } from "../../../services/BookService";
...
const UpdateBookPage = () => {
  ...
  const validate = () => {
    ...
    if (!formData.title.trim()) newErrors.title = "Vui lòng nhập tên truyện";
    if (!formData.author_id) newErrors.author_id = "Vui lòng chọn tác giả";
    if (!formData.artist_id) newErrors.artist_id = "Vui lòng chọn họa sĩ";
    ...
    const fetchData = async () => {
      try {
        const [
          ...
          getBookBySlug(slug),
          getPagedCategories({ page: 1, limit: 100 }),
          getPagedAuthors({ page: 1, limit: 100 }),
          getPagedArtists({ page: 1, limit: 100 }),
          getPagedStatus({ page: 1, limit: 100 }),
        ]);
        ...
        setFormData({
          ...
          title: book.title ?? "",
          another_name: book.another_name ?? "",
          author_id: book.author_id ?? "",
          artist_id: book.artist_id ?? "",
          status: book.status ?? "",
          description: book.description ?? "",
          category_id: book.categories?.map(c => c.id) ?? [],
        });
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === "category_id") {
        value.forEach(id => payload.append("category_id[]", id));
      } else {
